...
func (m *ConfigManager) Add(u *UserInput) (*Config, error) {
	...
	if c == nil {
		return nil, fmt.Errorf("invalid userinput")
	}
	// Set Path
...
func (m *ConfigManager) MigrateConfig() error {
	...
	for _, c := range cl {
		...
		if len(p) < 2 {
			...
			k := strings.Split(c.Path, string(os.PathSeparator))
			k[0] = string(os.PathSeparator)
			k[len(k)-1] = nn + ".yaml"
			if err := os.Rename(c.Path, path.Join(k...)); err != nil {
				return err
			}
		}
	}
